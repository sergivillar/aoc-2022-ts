...
const findPath = (startPos: Point, endPos: Point, blizzards, minute) => {
  const queue = new Queue();
  const visited = {};

  queue.enqueue([startPos, blizzards, minute]);
  ...
  while (queue.length) {
    ...
    const blizzardsPos = newBlizzards.map(([_, position]) => getKey(position));
    ...
    for (let i = 0; i < DIRECTIONS.length; i++) {
      ...
      if (next.x === endPos.x && next.y === endPos.y) {
        return { time: nextTime, blizzards: newBlizzards };
      }
...
const { time: timeFirstTrip, blizzards: blizzardFirstTrip } =
  findPath(START_POS, END_POSITION, BLIZZARDS, 0) || {};

const { time: timeSecondTrip, blizzards: blizzardSecondTrip } =
  findPath(END_POSITION, START_POS, blizzardFirstTrip, timeFirstTrip) || {};

const { time: timeThirdTrip } =
  findPath(START_POS, END_POSITION, blizzardSecondTrip, timeSecondTrip) || {};

console.log("Result -> ", timeThirdTrip);
