...
const AVAILABLE_SPACE_TOTAL = 70000000;
const UPDATE_REQUIRE = 30000000;
const AVAILABLE_SPACE = AVAILABLE_SPACE_TOTAL - (rootDir?.size || 0);
const SPACE_TO_FREE = UPDATE_REQUIRE - AVAILABLE_SPACE;

let total = 0;

const getResult = (node) => {
  node.children.forEach((child) => {
    if (child.children.length > 0 && child.size >= SPACE_TO_FREE) {
      if (total === 0) {
        total = child.size;
      }
      if (child.size < total) {
