...
const runBlueprint = (recipe: Recipe) => {
  ...
  while (queue.length && currentTime < MINUTES) {
    ...
    for (let i = 0; i < iterationsAtThisMinute; i++) {
      ...
      const prevBest = bestGeodes[String(currentTime)] ?? 0;
      if (currentResources[3] < prevBest) {
        continue;
      }
      bestGeodes[String(currentTime)] = currentResources[3];
...
const result = recipes.reduce((acc, curr) => acc * runBlueprint(curr), 1);
